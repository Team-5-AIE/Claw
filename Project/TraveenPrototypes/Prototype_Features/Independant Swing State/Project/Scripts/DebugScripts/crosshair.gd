...
func _set_crosshair_visibility():
	if crosshairs_enabled:
		if player.finite_state_machine.swing_enabled:
	...
	if Input.is_action_just_pressed("SpearExtra"):
		...
		match (control_config):
			...
			ControlConfig.HOLD:
				...
				if Input.is_action_pressed("SpearPrimary") && !player.finite_state_machine.swing_enabled:
					print("HOLD: Used spear")
					return "Spear"
				elif Input.is_action_just_released("SpearPrimary") && player.finite_state_machine.swing_enabled:
					match (spear_alt_use_config):
						SpearAltUseConfig.RELEASE:
